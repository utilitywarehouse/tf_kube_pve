...
data "ignition_file" "worker_kubelet_dropin" {
  ...
  content {
    content = templatefile("${path.module}/resources/kubelet-dropin.conf",
      {
        labels = "role=worker,topology.kubernetes.io/zone=${var.zone_mapping[var.worker_instance_list[count.index].pve_host]},uw.systems/pve-host=${var.worker_instance_list[count.index].pve_host}"
      }
    )
  }
}
